...
export class RegionService {
  ...
  private convertDateFromClient(region: IRegion): IRegion {
    const copy: IRegion = Object.assign({}, region, {
      saatdakika: region.saatdakika != null && region.saatdakika.isValid() ? region.saatdakika.toJSON() : null
    });
    return copy;
  }

  private convertDateFromServer(res: EntityResponseType): EntityResponseType {
    res.body.saatdakika = res.body.saatdakika != null ? moment(res.body.saatdakika) : null;
    return res;
  }

  private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    res.body.forEach((region: IRegion) => {
      region.saatdakika = region.saatdakika != null ? moment(region.saatdakika) : null;
    });
    return res;
  }
}
